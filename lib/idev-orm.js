...
const { getRepositoryClass, setRepositoryClass } = require('./repositories.js');
const { extend } = require('./class-extender.js');
...
module.exports = class IdevOrm {
  ...
  extendRepository(tableName, extension) {
    const Repository = getRepositoryClass(tableName);
    const ExtendedClass = extend(Repository, extension);

    setRepositoryClass(tableName, ExtendedClass);

    this._repositoriesFactories = null;
  }
